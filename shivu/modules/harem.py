...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    if not user or 'characters' not in user or not user['characters']:
        text = "😔 **You have not collected any characters yet!**\n\n🔥 **Start guessing characters to build your collection!**"
    ...
    # 🎟 Fetch banner-exclusive characters owned by the user
    banner_characters = []
        ...
        if 'characters' in banner:
            for char in banner['characters']:
                if char['id'] in [c['id'] for c in user['characters']]:  # If user owns this banner character
                    char['name'] += " 🛡️ *Exclusive*"  # Mark banner characters
    ...
    for key, characters in grouped_characters.items():
        ...
        harem_message += f"\n📌 **{key}** `{len(characters)}/{total_in_category}`\n"
        for character in characters:
            count = character_counts[character['id']]
            harem_message += f" ➜ `{character['id']}` **{character['name']}** ×{count}\n"

    # 📜 Pagination Buttons
    keyboard = [[InlineKeyboardButton(f"📜 View Collection ({len(all_characters)})", switch_inline_query_current_chat=f"collection.{user_id}")]]
    if total_pages > 1:
        nav_buttons = []
        if page > 0:
    ...
    # 🖼️ If No Favorite, Send Random Character
    if all_characters:
    ...
    await update.message.reply_text(harem_message, parse_mode="Markdown", reply_markup=reply_markup)
...
async def sort_collection(update: Update, context: CallbackContext) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("🔀 **Choose how you want to sort your collection:**", parse_mode="Markdown", reply_markup=reply_markup)
...
async def sort_callback(update: Update, context: CallbackContext) -> None:
    ...
    await query.edit_message_text(f"✅ Collection is now sorted by **{sort_by.capitalize()}**.\nUse `/collection` to view.", parse_mode="Markdown")
